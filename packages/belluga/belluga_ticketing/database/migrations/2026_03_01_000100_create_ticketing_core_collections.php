...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('ticket_promotions', function (Blueprint $collection): void {
            $collection->index(['event_id' => 1, 'status' => 1, '_id' => 1]);
            $collection->index(['scope_type' => 1, 'event_id' => 1, 'status' => 1, '_id' => 1]);
            $collection->index(['occurrence_id' => 1, 'status' => 1, '_id' => 1]);
            $collection->index(['ticket_product_id' => 1, 'status' => 1, '_id' => 1]);
            $collection->unique(['event_id' => 1, 'code' => 1]);
            $collection->index(['created_at' => -1]);
        });

        Schema::create('ticket_promotion_redemptions', function (Blueprint $collection): void {
            $collection->index(['promotion_id' => 1, 'principal_id' => 1, '_id' => 1]);
            $collection->index(['order_id' => 1, '_id' => 1]);
            $collection->unique(['order_id' => 1, 'promotion_id' => 1]);
            $collection->index(['created_at' => -1]);
        });
        ...
        Schema::create('ticket_unit_audit_events', function (Blueprint $collection): void {
            $collection->index(['event_id' => 1, 'occurrence_id' => 1, 'operation' => 1, '_id' => 1]);
            $collection->index(['source_ticket_unit_id' => 1, 'operation' => 1, '_id' => 1]);
            $collection->index(['target_ticket_unit_ids' => 1, 'operation' => 1, '_id' => 1]);
            $collection->index(['actor_ref.id' => 1, 'operation' => 1, '_id' => 1]);
            $collection->unique('idempotency_key');
            $collection->index(['created_at' => -1]);
        });
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('ticket_unit_audit_events');
        ...
        Schema::dropIfExists('ticket_promotion_redemptions');
