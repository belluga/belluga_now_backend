...
namespace Belluga\Ticketing\Application\Transactions;

use Belluga\Ticketing\Support\TicketingDomainException;
use Illuminate\Support\Facades\DB;
use Throwable;

class TenantTransactionRunner
{
    /**
     * @template T
     *
    ...
    public function run(callable $callback): mixed
    {
        ...
        if (! method_exists($connection, 'transaction')) {
            throw new TicketingDomainException(
                ...
                httpStatus: 500,
                message: 'Tenant MongoDB transaction support is required for ticketing critical flows.'
            );
        }
        ...
        try {
            ...
            if ($this->isTransactionSupportError($throwable)) {
                throw new TicketingDomainException(
                    errorCode: 'transaction_unavailable',
                    httpStatus: 500,
                    message: 'Configure replica set / transaction-capable runtime for ticketing critical flows.',
                );
            }
