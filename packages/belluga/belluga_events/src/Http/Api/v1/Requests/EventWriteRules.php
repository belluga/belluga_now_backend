...
final class EventWriteRules
{
    ...
    private static function build(bool $create): array
    {
        return [
            ...
            'occurrences.*.place_ref.*' => 'prohibited',
            'occurrences.*.event_parties' => 'prohibited',
            'occurrences.*.event_parties.*' => 'prohibited',
            'occurrences.*.event_parties.*.party_type' => 'prohibited',
            'occurrences.*.event_parties.*.party_ref_id' => 'prohibited',
            'occurrences.*.event_parties.*.permissions' => 'prohibited',
            'occurrences.*.event_parties.*.permissions.can_edit' => 'prohibited',
            'occurrences.*.event_parties.*.metadata' => 'prohibited',
            'occurrences.*.profile_groups' => 'sometimes|array|max:'.InputConstraints::EVENT_PROFILE_GROUPS_MAX,
            'occurrences.*.profile_groups.*' => 'array:id,key,label,order,account_profile_ids',
            'occurrences.*.profile_groups.*.id' => 'sometimes|nullable|string|max:'.InputConstraints::EVENT_PROFILE_GROUP_KEY_MAX.'|regex:/^[a-z0-9]+(?:[-_][a-z0-9]+)*$/',
            'occurrences.*.profile_groups.*.key' => 'sometimes|nullable|string|max:'.InputConstraints::EVENT_PROFILE_GROUP_KEY_MAX.'|regex:/^[a-z0-9]+(?:[-_][a-z0-9]+)*$/',
            'occurrences.*.profile_groups.*.label' => 'required_with:occurrences.*.profile_groups|string|max:'.InputConstraints::NAME_MAX,
            'occurrences.*.profile_groups.*.order' => 'sometimes|integer|min:0',
            'occurrences.*.profile_groups.*.account_profile_ids' => 'sometimes|array|max:'.InputConstraints::EVENT_PROFILE_GROUP_MEMBERS_MAX,
            'occurrences.*.profile_groups.*.account_profile_ids.*' => 'required_with:occurrences.*.profile_groups.*.account_profile_ids|string|size:'.InputConstraints::OBJECT_ID_LENGTH,
            ...
            'occurrences.*.programming_items.*.time' => 'sometimes|nullable|string|date_format:H:i',
            'occurrences.*.programming_items.*.end_time' => 'sometimes|nullable|string|date_format:H:i',
            'occurrences.*.programming_items.*.title' => 'sometimes|nullable|string|max:'.InputConstraints::RICH_TEXT_MAX_BYTES,
            ...
            'event_parties' => 'prohibited',
            'event_parties.*' => 'prohibited',
            'event_parties.*.party_type' => 'prohibited',
            'event_parties.*.party_ref_id' => 'prohibited',
            'event_parties.*.permissions' => 'prohibited',
            'event_parties.*.permissions.can_edit' => 'prohibited',
            'event_parties.*.metadata' => 'prohibited',
            'profile_groups' => 'sometimes|array|max:'.InputConstraints::EVENT_PROFILE_GROUPS_MAX,
            'profile_groups.*' => 'array:id,key,label,order,account_profile_ids',
            'profile_groups.*.id' => 'sometimes|nullable|string|max:'.InputConstraints::EVENT_PROFILE_GROUP_KEY_MAX.'|regex:/^[a-z0-9]+(?:[-_][a-z0-9]+)*$/',
            'profile_groups.*.key' => 'sometimes|nullable|string|max:'.InputConstraints::EVENT_PROFILE_GROUP_KEY_MAX.'|regex:/^[a-z0-9]+(?:[-_][a-z0-9]+)*$/',
            'profile_groups.*.label' => 'required_with:profile_groups|string|max:'.InputConstraints::NAME_MAX,
            'profile_groups.*.order' => 'sometimes|integer|min:0',
            'profile_groups.*.account_profile_ids' => 'sometimes|array|max:'.InputConstraints::EVENT_PROFILE_GROUP_MEMBERS_MAX,
            'profile_groups.*.account_profile_ids.*' => 'required_with:profile_groups.*.account_profile_ids|string|size:'.InputConstraints::OBJECT_ID_LENGTH,
        ];
    }
}
