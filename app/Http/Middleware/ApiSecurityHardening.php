...
class ApiSecurityHardening
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $rateLimited = $this->enforceRateLimit($request, $profile, $correlationId, $cfRayId);
        if ($rateLimited !== null) {
            return $rateLimited;
        }
        ...
        if ($this->isUnsafeMethod($request) && (bool) ($profile['require_idempotency'] ?? false)) {
            $idempotencyResult = $this->enforceIdempotency($request, $profile, $correlationId, $cfRayId);
